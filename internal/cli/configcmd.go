...
func configKeySpecs() []configKeySpec {
	...
	return []configKeySpec{
		...
		static(optionalBoolKey("scoring.enabled", "Whether completed reviews earn the PR's author points (default true)",
			func(c *config.Config) **bool { return &c.Scoring.Enabled }), boolValues),
