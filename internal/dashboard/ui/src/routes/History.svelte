<script lang="ts">
  ...
  import { ago, durSecs, prHref, tokens, usd, when } from '../lib/format';
      ...
      <div class="review-table">
        ...
        {#each reviews as r (rowKey(r))}
          ...
          <div class="review-row-wrap" class:open>
            ...
            >
              <span class="pr-cell"><PrIdentity repo={r.repo} number={r.number} title={r.title} author={r.author} score={r.score} bucket={r.score_bucket ?? ''} mine={isViewer(r.author, $viewer)} /></span>
            ...
            {#if open}
              <div class="review-detail">
                <dl>
                  ...
                  {#if r.cost_usd}
                    <div><dt>Cost</dt><dd>{usd(r.cost_usd)}{#if r.cost_estimated} <span class="tag-mute">estimated</span>{/if}</dd></div>
                  {/if}
                </dl>
                {#if r.steering}
