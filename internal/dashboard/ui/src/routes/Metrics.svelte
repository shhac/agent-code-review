...
{#if data}
  <div class="metrics-stack">
    <section class="metric-kpis">
      ...
      <div><strong>{tokens(data.summary.tokens_used) || '0'}</strong><span>tokens processed</span></div>
    ...
    <section class="surface metric-panel scatter-panel"><div class="section-head"><div><h2>Duration vs. tokens</h2><span>Each point is one completed review. Tokens exclude cached re-reads, so engines compare.</span></div><label class="colour-control">Colour by <select bind:value={colour}><option value="verdict">verdict</option><option value="model">model</option></select></label></div><div class="scatter" aria-label="Duration versus tokens scatter plot">
      ...
      {#if tip}<div class="scatter-tip" style={tip.style}><p class="tip-head"><i class={tip.cls}></i>{modelLabel(tip.point.model)} · {tip.point.effort || 'model default'}</p><p class="tip-vals"><b>{tokens(tip.point.tokens_used) || '?'}</b> tokens · <b>{durSecs(tip.point.duration_secs) || '?'}</b> · {statusLabel(tip.point.verdict)}</p></div>{/if}
    ...
    <section class="surface metric-panel"><div class="section-head"><h2>Model + effort breakdown</h2><span>CLI version retained per review</span></div><div class="metric-table"><p class="metric-table-head"><b>Model</b><b>Effort</b><b>Reviews</b><b>Tokens</b><b>Median</b><b>Cost</b><b>Version</b></p>{#each data.models as row}<p><span>{modelLabel(row.model)}</span><span>{row.effort || 'model default'}</span><span>{row.reviews}</span><span>{tokens(row.tokens_used) || '–'}</span><span>{durSecs(row.median_duration_secs) || '–'}</span><span>{usd(row.median_cost_usd) || '–'}</span><span class="mono">{row.engine_version || 'unavailable'}</span></p>{/each}</div></section>
  </div>
{/if}
