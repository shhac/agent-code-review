...
{#if data}
  <div class="metrics-stack">
    ...
    <section class="surface metric-panel scatter-panel"><div class="section-head"><div><h2>Duration vs. tokens</h2><span>Each point is one completed review.</span></div><label class="colour-control">Colour by <select bind:value={colour}><option value="verdict">verdict</option><option value="model">model</option></select></label></div><div class="scatter" aria-label="Duration versus tokens scatter plot">{#each data.scatter as point}{@const pointClass = scatterClass(point, colour as 'verdict' | 'model')}{@const pos = scatterPos(point, scatterDuration, scatterTokens)}<i class={pointClass} style={`left:${pos.left}; bottom:${pos.bottom}`} title={`${point.model || 'Codex default'} / ${point.effort || 'default'} · ${point.verdict} · ${tokens(point.tokens_used) || 'unknown'} tokens · ${durSecs(point.duration_secs) || 'unknown duration'}`}></i>{/each}<span class="axis x">duration →</span><span class="axis y">tokens →</span></div></section>
