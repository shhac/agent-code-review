...
func TestScoreLandsOnTheCompletedRow(t *testing.T) {
	...
	if !got.Score.Scored() || got.Score.Points() != 135 {
		t.Errorf("score = %+v, want 135", got.Score)
	}
...
func TestGeneratedFilesAreExcludedFromTheScoredCounts(t *testing.T) {
	...
	if got.Score.Points() != 135 {
		t.Errorf("score = %d, want the small-bucket 135 rather than a huge-bucket score", got.Score.Points())
	}
}
...
func TestAttemptIndexIsFrozenOntoTheRow(t *testing.T) {
	...
	// 100 * (45/50) * 1.5 * 1.0 * 0.6^2 = 48.6 -> 49
	if got.Score.Points() != 49 {
		t.Errorf("score = %d, want 49 (the third revision's decay)", got.Score.Points())
	}
}
