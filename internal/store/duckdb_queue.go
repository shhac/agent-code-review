...
func historyInsert(r Review) string {
	...
	return fmt.Sprintf(`INSERT INTO history (repo, number, title, author, head_sha, verdict, engine, model, effort, engine_version, reviewed_at, duration_secs, work_dir, tokens_used, cost_usd, est_cost_usd, fresh_tokens, input_tokens, output_tokens, cache_write_tokens, cache_read_tokens, reasoning_tokens, usage_raw, steering_message, steering_by, steering_at, policy_violation, additions, deletions, changed_files, scored_additions, scored_deletions, excluded_files, diff_sha, score, score_source, score_rules, score_bucket, score_note, score_attempt, scored_at, diff_files) VALUES (%s, %d, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, %d, %s, %s, %d, %d, %d, %d, %d, %d, %s, %s, %s, %s, %t, %d, %d, %d, %d, %d, %d, %s, %s, %s, %s, %s, %s, %s, %s, %s);`,
		...
		intOrNull(r.Score.Score), nullText(r.Score.Source), nullText(r.Score.Rules), nullText(r.Score.Bucket), nullText(r.Score.Note), intOrNull(r.Score.Attempt), ts(r.Score.At), nullText(marshalFiles(r.DiffFiles)))
}
