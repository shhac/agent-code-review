...
func TestTailscaleLoginIdentity(t *testing.T) {
	...
	must(Author{Repo: "*", GitHubHandle: "ada", Group: "approver", TailscaleLogin: "Ada@Example.com"})
	must(Author{Repo: "*", GitHubHandle: "grace", Group: "approver", Email: "grace@example.com"})
	...
	t.Run("resolves to the handle, case-insensitively", func(t *testing.T) {
		for _, probe := range []string{"Ada@Example.com", "ada@example.com", "ADA@EXAMPLE.COM"} {
			...
			if got.GitHubHandle != "ada" {
				t.Errorf("%s resolved to %q, want ada", probe, got.GitHubHandle)
			}
		}
	})

	t.Run("a row with no login never matches", func(t *testing.T) {
		// grace has an email but no tailscale login. Email must not stand in
		// for it: that is the whole reason the column is separate.
		if _, ok, err := s.AuthorByTailscaleLogin(ctx, "grace@example.com"); err != nil || ok {
	...
	t.Run("two people cannot claim one login", func(t *testing.T) {
		err := s.SetAuthorGroup(ctx, Author{
			...
			TailscaleLogin: "ada@example.com",
		})
		if err == nil {
	...
	t.Run("the owner may still update their own row", func(t *testing.T) {
		must(Author{Repo: "*", GitHubHandle: "ada", Group: "commenter", TailscaleLogin: "ada@example.com"})
		got, ok, err := s.AuthorByTailscaleLogin(ctx, "ada@example.com")
